...
Each directory is a complete GraineSQL project: a config, a schema per engine, the
...
committed seed files. This ties the fixture to a source GraineSQL never wrote, so
...
`UPDATE_FIXTURES=1` env var) rebuilds them from `data.sql` through GraineSQL
...
gone before GraineSQL ever sees the value. The fixtures that share files across all
